// src/components/search.js - 集成详情配置管理的统一搜索组件
...
import detailConfigAPI from '../services/detail-config-api.js'; // 新增：详情配置API
...
export class UnifiedSearchManager {
  constructor() {
    ...
    this.config = {
      ...
      // 详情提取配置 - 这些将从detailConfigAPI动态获取
  ...
  setupConfigChangeListeners() {
    // 监听详情配置变更事件
    document.addEventListener('detailConfigChanged', (event) => {
      console.log('检测到详情配置变更，更新搜索组件配置');
      this.handleConfigChange(event.detail.config, event.detail.hasUnsavedChanges);
    });

    // 监听详情配置保存事件
    document.addEventListener('detailConfigSaved', (event) => {
      console.log('详情配置已保存，刷新搜索组件配置');
      this.loadUserConfig(); // 重新加载最新配置
    });
   ...
   * 处理配置变更 - 新增方法
   */
  handleConfigChange(newConfig, hasUnsavedChanges) {
    // 只有保存后的配置才应用到搜索组件
    if (!hasUnsavedChanges) {
      this.updateConfigFromDetailConfig(newConfig);
    }
  }
  ...
  updateConfigFromDetailConfig(detailConfig) {
    const oldExtractionEnabled = this.config.enableDetailExtraction;
    
    // 更新搜索组件配置
    this.config.enableDetailExtraction = detailConfig.enableDetailExtraction;
    this.config.autoExtractDetails = detailConfig.autoExtractDetails;
    this.config.maxAutoExtractions = detailConfig.maxAutoExtractions;
    this.config.extractionBatchSize = detailConfig.extractionBatchSize;
    this.config.extractionTimeout = detailConfig.extractionTimeout;
    this.config.enableRetry = detailConfig.enableRetry;
    this.config.maxRetryAttempts = detailConfig.maxRetryAttempts;
    this.config.enableCache = detailConfig.enableCache;
    this.config.showExtractionProgress = detailConfig.showExtractionProgress;
    this.config.showScreenshots = detailConfig.showScreenshots;
    this.config.showDownloadLinks = detailConfig.showDownloadLinks;
    this.config.showMagnetLinks = detailConfig.showMagnetLinks;
    this.config.showActressInfo = detailConfig.showActressInfo;
    this.config.compactMode = detailConfig.compactMode;
    this.config.enableImagePreview = detailConfig.enableImagePreview;
    this.config.strictValidation = detailConfig.enableStrictDomainCheck;
    this.config.enableContentFilter = detailConfig.enableContentFilter;
    this.config.contentFilterKeywords = detailConfig.contentFilterKeywords;

    // 如果详情提取功能状态发生变化，更新UI提示
    if (oldExtractionEnabled !== this.config.enableDetailExtraction) {
      if (this.config.enableDetailExtraction) {
        showToast('详情提取功能已启用', 'info');
      } else {
        showToast('详情提取功能已禁用', 'info');
      }
    }
    ...
    try {
      if (authManager.isAuthenticated()) {
        ...
        if (this.isConfigCacheValid()) {
          console.log('使用缓存的详情配置');
          this.updateConfigFromDetailConfig(this.configCache);
          return;
        }
        ...
        // 更新详情提取配置
        this.updateConfigFromDetailConfig(detailConfigData.config);
        
        // 缓存配置
        this.configCache = detailConfigData.config;
  ...
  exposeGlobalMethods() {
    window.unifiedSearchManager = {
      ...
      isDetailExtractionEnabled: () => this.config.enableDetailExtraction,
      updateDisplayConfig: (displayConfig) => {
        // 只更新显示相关的配置
        if (displayConfig.compactMode !== undefined) {
          this.config.compactMode = displayConfig.compactMode;
        }
        if (displayConfig.showScreenshots !== undefined) {
          this.config.showScreenshots = displayConfig.showScreenshots;
        }
        if (displayConfig.showDownloadLinks !== undefined) {
          this.config.showDownloadLinks = displayConfig.showDownloadLinks;
        }
        if (displayConfig.showMagnetLinks !== undefined) {
          this.config.showMagnetLinks = displayConfig.showMagnetLinks;
        }
        if (displayConfig.showActressInfo !== undefined) {
          this.config.showActressInfo = displayConfig.showActressInfo;
        }
        console.log('搜索组件显示配置已更新:', this.config);
      }
    };
  ...
  getServiceStatus() {
    return {
      ...
      features: {
        detailExtraction: this.config.enableDetailExtraction,
        autoExtraction: this.config.autoExtractDetails,
  ...
  getExtractionCapabilities() {
    return {
      enabled: this.config.enableDetailExtraction,
      authenticated: authManager.isAuthenticated(),
      supportedSources: APP_CONSTANTS.DETAIL_EXTRACTION_SOURCES || [],
      maxAutoExtractions: this.config.maxAutoExtractions,
      batchSize: this.config.extractionBatchSize,
      timeout: this.config.extractionTimeout,
      concurrency: this.config.enableConcurrentExtraction ? this.config.maxConcurrentExtractions : 1,
      caching: this.config.enableCache,
      retry: this.config.enableRetry,
  ...
  exportSearchConfig() {
    const configData = {
      ...
      detailExtractionConfig: {
        enableDetailExtraction: this.config.enableDetailExtraction,
        autoExtractDetails: this.config.autoExtractDetails,
        maxAutoExtractions: this.config.maxAutoExtractions,
  ...
  async refreshDetailConfig() {
    try {
      ...
      // 触发配置更新事件
      document.dispatchEvent(new CustomEvent('searchConfigRefreshed', {
        detail: { config: this.config }
      }));
      
    } catch (error) {
      console.error('刷新详情配置失败:', error);
      showToast('配置刷新失败: ' + error.message, 'warning');
    }
  }
