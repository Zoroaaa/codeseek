  ...
  <div className="collapsible-section animate-fade-in" style={{ animationDelay: '200ms' }}>
    <button onClick={onToggle} className="collapsible-header">
      <div className="flex items-center gap-2.5 sm:gap-3">
        <div className="w-8 h-8 sm:w-9 sm:h-9 rounded-xl bg-violet-100 dark:bg-violet-900/30 flex items-center justify-center">
          <Globe className="w-4 h-4 sm:w-4.5 sm:h-4.5 text-violet-600 dark:text-violet-400" />
        </div>
        <span className="font-semibold text-surface-900 dark:text-surface-100 text-sm sm:text-base lg:text-lg">搜索源管理</span>
        <span className="px-2.5 py-1 text-xs font-semibold bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300 rounded-full">
          {allSources.length}
        </span>
      </div>
      <div className="flex items-center gap-1.5 sm:gap-2">
        {show && (
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 text-xs font-medium rounded-xl bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-300 hover:bg-primary-200 dark:hover:bg-primary-900/50 transition-all disabled:opacity-50"
          >
            {isBatchChecking ? (
              <><RefreshCw className="w-3.5 h-3.5 animate-spin" />检查中</>
            ) : (
              <><Activity className="w-3.5 h-3.5" />批量检查</>
            )}
          </button>
        )}
    ...
    {show && (
      <div className="border-t border-surface-100 dark:border-surface-800 max-h-[420px] sm:max-h-[550px] lg:max-h-[650px] overflow-y-auto scrollbar-thin">
          ...
          return (
            <div key={majorCategory.id} className="border-b border-surface-50 dark:border-surface-800/60 last:border-b-0">
              <button
                ...
                className="w-full flex items-center justify-between px-4 sm:px-5 lg:px-6 py-3 sm:py-4 hover:bg-surface-50 dark:hover:bg-surface-800/40 transition-colors"
              >
                <div className="flex items-center gap-2.5 sm:gap-3">
                  <ChevronRight className={`w-4 h-4 sm:w-4.5 sm:h-4.5 text-surface-400 transition-transform duration-200 ${isMajorExpanded ? 'rotate-90' : ''}`} />
                  <div
                    className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-white shadow-md"
                    style={{ backgroundColor: majorCategory.color || '#6366f1' }}
                  >
                    {majorCategory.icon ? (
                      <span className="text-lg sm:text-xl">{majorCategory.icon}</span>
                    ) : (
                      <Database className="w-4 h-4 sm:w-5 sm:h-5" />
                    )}
                  </div>
                  <div className="text-left">
                    <span className="text-sm sm:text-base font-semibold text-surface-800 dark:text-surface-200">{majorCategory.name}</span>
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  {majorCategory.requiresKeyword ? (
                    <span className="text-[10px] sm:text-xs px-2.5 py-1 bg-surface-100 dark:bg-surface-800 text-surface-500 dark:text-surface-400 rounded-lg font-medium">
                      {enabledSources}/{totalSources} 启用
                    </span>
                  ) : (
                    <span className="text-[10px] sm:text-xs px-2.5 py-1 bg-surface-100 dark:bg-surface-800 text-surface-400 dark:text-surface-500 rounded-lg font-medium">
                      浏览型
                    </span>
                  )}
                </div>
                    ...
                    return (
                      <div key={category.id}>
                        <button
                          ...
                          className="w-full flex items-center justify-between px-4 sm:px-5 lg:px-6 py-2.5 sm:py-3 pl-10 sm:pl-14 hover:bg-surface-100/50 dark:hover:bg-surface-800/30 transition-colors"
                        >
                          <div className="flex items-center gap-2 sm:gap-3">
                            <ChevronRight className={`w-3.5 h-3.5 sm:w-4 sm:h-4 text-surface-400 transition-transform duration-200 ${isCategoryExpanded ? 'rotate-90' : ''}`} />
                            <div
                        ...
                        {isCategoryExpanded && (
                          <div className="px-3 sm:px-4 lg:px-5 pb-3 sm:pb-4 pl-12 sm:pl-18">
                                    ...
                                    <div className="flex items-start justify-between gap-2 sm:gap-2.5">
                                      <div className="flex items-start gap-2 sm:gap-2.5 min-w-0 flex-1">
                                        ...
                                        <div className="min-w-0 flex-1">
                                          <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap">
                                            <span className={`text-xs sm:text-sm font-semibold ${isEnabled ? 'text-surface-800 dark:text-surface-200' : 'text-surface-500'}`}>
                                              {sourceName}
                                            </span>
                                            <span className={`text-[9px] sm:text-[10px] px-1.5 py-0.5 rounded font-medium ${getSiteTypeBadge(source.siteType)}`}>
                                              {getSiteTypeLabel(source.siteType)}
                                            </span>
                                            {majorCategory.requiresKeyword && (
                                              <span className={`flex items-center gap-0.5 text-[9px] sm:text-[10px] px-1.5 py-0.5 rounded font-medium ${
                                                isEnabled
                                                  ? 'bg-success-100 text-success-700 dark:bg-success-900/30 dark:text-success-400'
                                                  : 'bg-surface-200 text-surface-500 dark:bg-surface-700 dark:text-surface-400'
                                              }`}>
                                                {isEnabled ? <CheckCircle className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> : <XCircle className="w-2.5 h-2.5 sm:w-3 sm:h-3" />}
                                                {isEnabled ? '启用' : '禁用'}
                                              </span>
                                            )}
                                            {checkResult && (() => {
                                              ...
                                              return (
                                                <span
                                                  className={`flex items-center gap-0.5 text-[9px] sm:text-[10px] px-1.5 py-0.5 rounded font-medium ${colorClass}`}
                                                  title={titleText}
                                                >
                                                  {checkResult.available
                                                    ? <><Zap className="w-2.5 h-2.5 sm:w-3 sm:h-3" />{label}</>
                                                    : <><XCircle className="w-2.5 h-2.5 sm:w-3 sm:h-3" />{label}</>
                                                  }
                                                </span>
                                              );
                                            })()}
                                      ...
                                      <div className="flex items-center gap-0.5 shrink-0">
                                        <button
                                          ...
                                          className="p-1.5 rounded-xl text-surface-400 hover:text-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-all"
                                          title="健康检查"
                                        >
