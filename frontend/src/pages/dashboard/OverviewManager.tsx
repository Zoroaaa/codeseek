      ...
      <div className="p-4 sm:p-5 lg:p-6">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-xs sm:text-sm text-surface-500 dark:text-surface-400 mb-1">{title}</p>
            <p className="text-2xl sm:text-3xl font-bold text-surface-900 dark:text-surface-100">{value}</p>
            {change !== undefined && (
              <div className={`flex items-center gap-1 mt-2 text-xs sm:text-sm ${change >= 0 ? 'text-success-600' : 'text-error-600'}`}>
                {change >= 0 ? <ArrowUpRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> : <ArrowDownRight className="w-3.5 h-3.5 sm:w-4 sm:h-4" />}
                <span>{Math.abs(change)}%</span>
                <span className="text-surface-400 hidden sm:inline">vs 上周</span>
              </div>
            )}
          </div>
          <div className={`w-11 h-11 sm:w-12 sm:h-14 rounded-xl bg-gradient-to-br ${colorStyles[color]} flex items-center justify-center text-white shadow-lg flex-shrink-0`}>
            {icon}
          </div>
        </div>
      </div>
...
const QuickAction: React.FC<QuickActionProps> = ({ icon, title, description, onClick, gradient }) => (
  ...
  >
    <div className={`icon-badge ${gradient} text-white shadow-md`}>
      {icon}
    </div>
    <div className="min-w-0">
      <p className="font-medium text-surface-900 dark:text-surface-100 text-sm sm:text-base">{title}</p>
      <p className="text-xs sm:text-sm text-surface-500 dark:text-surface-400 truncate">{description}</p>
    </div>
  </button>
);
    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center shadow-lg shadow-primary-500/25">
      ...
      <div className="grid gap-4 sm:gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2" padding="none">
          <div className="p-4 sm:p-5 lg:p-6">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <h3 className="text-base sm:text-lg font-semibold text-surface-900 dark:text-surface-100 flex items-center gap-2">
          ...
          {recentActivities.length > 0 ? (
            <div className="px-4 sm:px-5 lg:px-6 pb-4 sm:pb-5 lg:pb-6 space-y-3">
              {recentActivities.slice(0, 5).map((activity) => (
                <div key={activity.id} className="flex items-center gap-3 sm:gap-4 p-3 rounded-xl hover:bg-surface-50 dark:hover:bg-surface-800/50 transition-colors">
                  <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${getActivityColor(activity.action)}`}>
                    {getActivityIcon(activity.action)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm font-medium text-surface-900 dark:text-surface-100 truncate">
            ...
            <div className="px-4 sm:px-5 lg:px-6 pb-4 sm:pb-5 lg:pb-6 text-center py-8">
              <Activity className="w-10 h-10 sm:w-12 sm:h-12 mx-auto text-surface-300 dark:text-surface-600 mb-3" />
              <p className="text-sm text-surface-500 dark:text-surface-400">暂无活动记录</p>
            </div>
          )}
        </Card>
        ...
        <Card padding="none">
          <div className="p-4 sm:p-5 lg:p-6">
            <h3 className="text-base sm:text-lg font-semibold text-surface-900 dark:text-surface-100 mb-4 flex items-center gap-2">
        ...
        {(() => {
          ...
          return (
            <Card padding="none" className="overflow-hidden relative">
              <div className="absolute top-0 right-0 w-32 h-32 sm:w-40 sm:h-40 rounded-full bg-primary-50 dark:bg-primary-900/20 -translate-y-12 sm:-translate-y-16 translate-x-12 sm:translate-x-16 pointer-events-none" />
              <div className="p-4 sm:p-5 lg:p-6">
                <h3 className="text-base sm:text-lg font-semibold text-surface-900 dark:text-surface-100 mb-4 flex items-center gap-2">
                ...
                <div className={`relative rounded-2xl bg-gradient-to-br ${level.color} p-4 sm:p-5 text-white mb-4`}>
                  <div className="flex items-center gap-3 mb-3">
                    <span className="text-2xl sm:text-3xl">{level.icon}</span>
                    <div>
                      <p className="text-[10px] sm:text-xs text-white/70 font-medium">当前等级</p>
                      <p className="text-xl sm:text-2xl font-bold">{level.label}</p>
                    </div>
                    <div className="ml-auto text-right">
                      <p className="text-[10px] sm:text-xs text-white/70">累计操作</p>
                ...
                <div className="grid grid-cols-3 gap-2 sm:gap-3">
                  <div className="text-center p-2.5 sm:p-3 bg-surface-50 dark:bg-surface-800/50 rounded-xl">
                    <p className="text-base sm:text-lg font-bold text-surface-900 dark:text-surface-100">{userStats.searchCount}</p>
                    <p className="text-[10px] sm:text-xs text-surface-500 dark:text-surface-400 mt-0.5">搜索次数</p>
                  </div>
                  <div className="text-center p-2.5 sm:p-3 bg-surface-50 dark:bg-surface-800/50 rounded-xl">
                    <p className="text-base sm:text-lg font-bold text-surface-900 dark:text-surface-100">{userStats.favoriteCount}</p>
                    <p className="text-[10px] sm:text-xs text-surface-500 dark:text-surface-400 mt-0.5">收藏数量</p>
                  </div>
                  <div className="text-center p-2.5 sm:p-3 bg-surface-50 dark:bg-surface-800/50 rounded-xl">
                    <p className="text-base sm:text-lg font-bold text-surface-900 dark:text-surface-100">
                      {new Set(searchHistory.map(h => h.query)).size}
                    </p>
                    <p className="text-[10px] sm:text-xs text-surface-500 dark:text-surface-400 mt-0.5">不同词</p>
                  </div>
                </div>
              </div>
            </Card>
            ...
            {searchHistory.length > 0 && (
              <div>
                <div className="flex items-center gap-1.5 mb-2">
                  ...
                  <span className="text-[10px] sm:text-xs text-surface-500 dark:text-surface-400 font-medium">最近搜索</span>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {[...new Set(searchHistory.slice(0, 10).map(h => h.query))].slice(0, 8).map(q => (
                    <button
                      key={q}
                      onClick={() => navigate('/main')}
                      className="text-[10px] sm:text-xs px-2 sm:px-2.5 py-1 rounded-full bg-surface-100 dark:bg-surface-800 text-surface-600 dark:text-surface-400 hover:bg-primary-50 hover:text-primary-600 dark:hover:bg-primary-900/30 dark:hover:text-primary-400 transition-colors border border-surface-200 dark:border-surface-700 truncate max-w-[100px] sm:max-w-[120px]"
                    >
                      {q}
                    </button>
                  ))}
