...
CREATE TABLE detail_extraction_config (
    ...
    max_download_links INTEGER DEFAULT 10,
    max_magnet_links INTEGER DEFAULT 10,
    max_screenshots INTEGER DEFAULT 10,
    
    -- 时间控制
    extraction_timeout INTEGER DEFAULT 15000,
    cache_duration INTEGER DEFAULT 86400000,
    ...
    show_extracted_tags INTEGER DEFAULT 1,
    ...
    enable_progress_notifications INTEGER DEFAULT 1,
    ...
    -- 高级选项
    enable_strict_domain_check INTEGER DEFAULT 1,
    enable_spam_filter INTEGER DEFAULT 1,
    prefer_original_sources INTEGER DEFAULT 1,
    enable_auto_code_extraction INTEGER DEFAULT 1,
    
    -- 性能优化
    enable_concurrent_extraction INTEGER DEFAULT 1,
    max_concurrent_extractions INTEGER DEFAULT 3,
    enable_smart_batching INTEGER DEFAULT 1,
    
    -- 数据质量
    require_minimum_data INTEGER DEFAULT 1,
    skip_low_quality_results INTEGER DEFAULT 0,
    validate_image_urls INTEGER DEFAULT 1,
    validate_download_links INTEGER DEFAULT 1,
...
-- 创建索引
CREATE INDEX idx_detail_config_user ON detail_extraction_config(user_id);
CREATE INDEX idx_detail_config_updated ON detail_extraction_config(updated_at DESC);

-- 创建触发器
CREATE TRIGGER update_detail_config_timestamp 
...
-- 创建配置预设索引
CREATE INDEX idx_config_presets_system ON detail_config_presets(is_system_preset);
CREATE INDEX idx_config_presets_public ON detail_config_presets(is_public);
CREATE INDEX idx_config_presets_usage ON detail_config_presets(usage_count DESC);
CREATE INDEX idx_config_presets_created ON detail_config_presets(created_at DESC);
